...
namespace fproject
{
    public partial class Form12 : Form
    {
        ...
        {
            InitializeComponent();
            // Set the form to full screen (optional)
        ...
        private void UpdateData()
        {
            ...
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                try
                {
                    ...
                    string query = "UPDATE IFlights SET Arlname = @Arlname, Dairport = @Dairport, Aairport = @Aairport, Dcountry = @Dcountry, Acountry = @Acountry, " +
                                   "Dcity = @Dcity, Acity = @Acity, Ddate = @Ddate, Adate = @Adate, Dtime = @Dtime, Atime = @Atime, Tprice = @Tprice WHERE FID = @FID";
                    using (SqlCommand cmd = new SqlCommand(query, conn))
                    {
                        ...
                        {
                            MessageBox.Show("No record found with the specified FID.");
                        }
                    }
                }
                catch (Exception ex)
